...
var ESConnector = function (settings, dataSource) {
    ...
    this.searchIndex = settings.searchIndex || settings.index || '';
    this.writeIndex = settings.writeIndex || false;
...
ESConnector.prototype.create = function (model, data, done) {
    ...
    document.index = this.getIndexForWrite();
...
ESConnector.prototype.destroy = function destroy(modelName, id, done) {
    ...
    this.resolveIndexForModel(modelName, id).then(
        function (index) {
            if (index) {
                filter.index = index;
            }
 ...
 * @param {Function} done callback
 */
ESConnector.prototype.save = function (modelName, data, done) {
    var self = this;
    if (self.debug) {
        log('ESConnector.prototype.save ', 'model', modelName, 'data', data);
    }

    var document = self.addDefaults(modelName);
    document[self.idField] = data.id;
    document.index = this.getIndexForWrite();
    if (!document[self.idField]) {
        throw new Error('Document id not setted!');
    }

    this.resolveIndexForModel(modelName, data.id).then(
        function (index) {
            if (index) {
                document.index = index;
            }

            document.body = self.matchDataToModel(modelName, {doc: data}, data.id, 'id');
            self.db.update(
                document
            ).then(
                function (response) {
...
ESConnector.prototype.updateOrCreate = function updateOrCreate(modelName, data, callback) {
    ...
    var defaults = self.addDefaults(modelName);
    defaults.index = this.searchIndex;
    self.db.update(_.defaults({
        id: id,
        body: {
            doc: data,
...
/**
 * Get index-name for WRITE operations
 * @return {*|string}
 */
ESConnector.prototype.getIndexForWrite = function () {
    ...
    try {
        ...
        throw new Error('This feature requires `moment` Package. Did you forgot to install it?')
    }

    return moment().format(this.writeIndex);
};

/**
 * Resolve the correct index of a Model for write-operations
 * @param modelName
 * @param id
 */
ESConnector.prototype.resolveIndexForModel = function (modelName, id) {
    var self = this;
    return new Promise(function (resolve, reject) {
        ...
        self.db.search({
            ...
            return resolve(resp.hits.hits[0]._index);
        }, function (err) {
            return reject(err);
        })
    });
