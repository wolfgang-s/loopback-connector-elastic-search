...
ESConnector.prototype.getClientConfig = function () {
    ...
    var config = {
        ...
        sniffOnConnectionFault: this.settings.sniffOnConnectionFault,
...
ESConnector.prototype.addDefaults = function (modelName, functionName) {
    ...
    if(['create', 'save', 'destroy', 'destroyAll', 'updateAttributes', 'updateOrCreate'].indexOf(functionName) != -1) {
      filter.refresh = true;
    }
    ...
    if (_.keys(where).length===0) {
        ...
        _.forEach(where, function(value, key) {
            ...
            if (spec) {
                ...
                if (key === 'like') {
                    var regexpQuery = {regexp:{}};
                    regexpQuery.regexp[spec] = cond.replace(new RegExp('%', 'g'), '.*');
                    body.query.bool.must.push(regexpQuery);
                }
